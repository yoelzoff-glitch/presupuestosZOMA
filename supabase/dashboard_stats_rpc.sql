...
CREATE OR REPLACE FUNCTION get_dashboard_stats(company_id_param UUID, days_filter INT DEFAULT 30)
RETURNS JSON AS $$
DECLARE
...
BEGIN
    ...
    WITH stats AS (
        SELECT
            ...
            (SELECT COALESCE(SUM(debit - credit), 0) FROM account_movements WHERE company_id = company_id_param) as total_balance,
...
DECLARE
  profile RECORD;
BEGIN
  FOR profile IN SELECT id, role, accepted_terms_version, company_id FROM public.users_profiles LOOP
    UPDATE auth.users
    SET raw_app_meta_data = COALESCE(raw_app_meta_data, '{}'::jsonb) || 
      jsonb_build_object(
        'role', profile.role, 
