 ...
 *
 * Security fix:
 *   - Verifies the authenticated user (Customer).
 *   - Ensures they only create notifications for their own company.
 */
export async function POST(req: NextRequest) {
  try {
    ...
    // Enforcement: Use the company_id from the user profile
    ...
    const { error } = await supabaseAdmin.from('notifications').insert({
      ...
      message: `${customerName || 'Un cliente'} envió el pedido ${orderCode} desde el portal.`,
      type: 'order',
