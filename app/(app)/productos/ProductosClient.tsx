...
export default function ProductosClient({ productosIniciales, idEmpresa, enableStockModule, businessType = 'products' }: Props) {
  ...
  return (
    <div className="min-h-screen w-full max-w-full overflow-x-hidden space-y-3 pb-6">
        ...
        <div className="relative z-10 flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
          <div className="min-w-0">
            <div className="mb-2 inline-flex items-center gap-2 rounded-full border border-white/10 bg-white/10 px-3 py-1 text-[10px] font-bold uppercase tracking-widest text-blue-200">
              <Package size={13} />
              {businessType === 'services' ? 'Servicios' : 'Productos'}
            </div>
            <h1 className="truncate text-2xl font-black tracking-tight">
              {businessType === 'services' ? 'Lista de servicios' : 'Lista de productos'}
            </h1>
            <p className="mt-1 line-clamp-1 text-xs text-slate-300">
              {businessType === 'services'
                ? 'Gestioná catálogo, precios y categorías de tus servicios.'
                : 'Gestioná catálogo, proveedores, categorías y precios.'}
            </p>
          </div>

          <div className="flex flex-wrap gap-2 lg:justify-end">
            <Link href="/productos/nuevo" className="inline-flex items-center justify-center gap-2 rounded-xl bg-blue-600 px-4 py-2 text-xs font-bold text-white shadow-lg shadow-blue-900/30 transition hover:bg-blue-500">
              <Plus size={16} /> Nuevo
            </Link>
      ...
      <section className={`grid w-full max-w-full grid-cols-1 gap-3 ${businessType === 'services' ? 'sm:grid-cols-2 lg:grid-cols-2' : 'sm:grid-cols-2 lg:grid-cols-5'}`}>
        <TarjetaEstado
          titulo={businessType === 'services' ? "Servicios" : (activeTab === 'venta' ? "Productos" : "Insumos")}
          valor={productosActuales.length}
          icon={businessType === 'services' ? Package : (activeTab === 'venta' ? Package : Boxes)}
          cargando={cargando}
          tono="blue"
        />
        {businessType !== 'services' && (
      ...
      <section className="w-full max-w-full overflow-hidden rounded-[1.25rem] border border-slate-200 bg-white shadow-sm">
            ...
            <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
              <div className="min-w-0">
                ...
                <p className="text-xs text-slate-500">
                  {businessType === 'services'
                    ? 'Buscá por nombre, código o categoría.'
        ...
        <div className="w-full max-w-full overflow-x-auto">
          {cargando ? <EstadoCargando /> : productosFiltrados.length === 0 ? <EstadoVacio tieneBusqueda={Boolean(busqueda.trim())} businessType={businessType} /> : (
            <div className={businessType === 'services' ? 'min-w-[600px]' : 'min-w-[900px]'}>
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    <CabeceraTabla>{businessType === 'services' ? 'Servicio' : 'Producto'}</CabeceraTabla>
                    <CabeceraTabla>Código</CabeceraTabla>
                    {businessType !== 'services' && <CabeceraTabla>Proveedor</CabeceraTabla>}
                    <CabeceraTabla>Categoría</CabeceraTabla>
                    {enableStockModule && businessType !== 'services' && <CabeceraTabla alineacion="right">Stock</CabeceraTabla>}
                    <CabeceraTabla alineacion="right">P. Venta</CabeceraTabla>
                    {businessType !== 'services' && <CabeceraTabla alineacion="right">P. Costo</CabeceraTabla>}
                ...
                <tbody className="divide-y divide-slate-100">
                  {productosPaginados.map((producto) => (
                    <tr key={producto.id} className="h-[52px] transition hover:bg-blue-50/40">
                      <td className="min-w-0 px-4 py-2"><IdentidadProducto producto={producto} businessType={businessType} /></td>
                      <td className="min-w-0 px-4 py-2"><EtiquetaCodigo codigo={producto.internal_code} /></td>
                      {businessType !== 'services' && <td className="min-w-0 px-4 py-2"><EtiquetaProveedor proveedor={producto.supplier} /></td>}
...
function IdentidadProducto({ producto, businessType }: { producto: Producto; businessType?: string }) {
  return (
    <div className="flex min-w-0 items-center gap-2">
      ...
      <div className="min-w-0">
        ...
        <p className="truncate text-[11px] font-semibold text-slate-400">
          {businessType === 'services' ? 'Servicio del catálogo' : 'Producto del catálogo'}
        </p>
      </div>
    </div>
  )
...
function EstadoVacio({ tieneBusqueda, businessType }: { tieneBusqueda: boolean; businessType?: string }) {
  return (
    <div className="flex min-h-[320px] flex-col items-center justify-center text-center">
      ...
      <h3 className="text-base font-black text-slate-900">
        {businessType === 'services' ? 'No hay servicios para mostrar' : 'No hay productos para mostrar'}
      </h3>
      <p className="mt-1 text-xs text-slate-500">
        {tieneBusqueda
          ? 'Probá cambiar la búsqueda o limpiarla.'
          : (businessType === 'services'
              ? 'Cargá un servicio nuevo para empezar.'
