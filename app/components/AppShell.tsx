...
function getPageDescription(pathname: string, businessType?: string) {
  ...
  if (pathname.startsWith('/tesoreria')) return 'Centro de control financiero: balance, compras, deudas y cobros'
...
function SidebarContent({ onNavigate }: { onNavigate?: () => void }) {
  ...
  let baseNavItems = navItems.filter(item => {
    if (businessType === 'services') {
      if (item.href === '/inventario' || item.href === '/cuenta-corriente' || item.href === '/tesoreria') {
        return false
      }
    } else {
      if (item.href === '/abonos') {
  ...
  if (isContador) {
    ...
    const clientesIdx = finalNavItems.findIndex(i => i.href === '/clientes')
    if (clientesIdx !== -1) {
      finalNavItems.splice(clientesIdx + 1, 0, {
        href: '/vendedores',
        label: 'Vendedores',
        icon: Users,
        isProFeature: true
