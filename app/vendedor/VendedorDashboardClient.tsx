  ...
  async function actualizarEstadisticas() {
    ...
    try {
      const { data: statsData, error } = await supabase.rpc('get_dashboard_stats', {
        company_id_param: idEmpresa,
        days_filter: 30,
        seller_id_param: rol === 'admin' ? null : idUsuario
      })
  ...
  return (
    <div className="space-y-8">
      ...
      <section className="grid grid-cols-1 md:grid-cols-3 gap-6">
        <TarjetaEstado 
          icon={Users} 
          titulo={rol === 'admin' ? "Total Clientes" : "Mis Clientes"} 
          valor={cargando ? '...' : estadisticas.clientes} 
          href="/vendedor/clientes"
          color="bg-purple-600"
        />
        <TarjetaEstado 
          icon={FileText} 
          titulo={rol === 'admin' ? "Total Presupuestos" : "Mis Presupuestos"} 
          valor={cargando ? '...' : estadisticas.presupuestos} 
          href="/vendedor/presupuestos"
          color="bg-blue-600"
        />
        <TarjetaEstado 
          icon={ClipboardList} 
          titulo={rol === 'admin' ? "Total Pedidos" : "Mis Pedidos"} 
          valor={cargando ? '...' : estadisticas.pedidos} 
      ...
      <section className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        <div className="bg-white p-7 rounded-[2rem] border border-slate-200 shadow-sm">
          ...
          <div className="grid grid-cols-2 gap-4">
            <AccionRapida href="/vendedor/clientes/nuevo" etiqueta="Crear Cliente" />
            <AccionRapida href="/vendedor/presupuestos" etiqueta="Ver Listado" />
          </div>
        </div>
...
function TarjetaEstado({ icon: Icon, titulo, valor, href, color }: any) {
  return (
    <Link href={href} className="bg-white p-7 rounded-[2rem] border border-slate-200 shadow-sm hover:shadow-md transition group">
      <div className="flex items-center gap-5">
        ...
        <div>
          <p className="text-[10px] font-black uppercase tracking-widest text-slate-400 mb-1">{titulo}</p>
...
function AccionRapida({ href, etiqueta }: { href: string, etiqueta: string }) {
  return (
    <Link href={href} className="flex items-center justify-between p-4 rounded-2xl bg-slate-50 border border-slate-100 hover:bg-blue-50 hover:border-blue-200 transition group">
      <span className="text-sm font-black text-slate-700 group-hover:text-blue-700">{etiqueta}</span>
